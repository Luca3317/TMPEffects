...
#pragma warning restore RS2008 // Enable analyzer release tracking
            ...
            IFieldSymbol varSymbol)
        {
            ...
            if (varSymbol.GetAttributes().Any(attr =>
                    attr.AttributeClass?.ToDisplayString() == Strings.SerializeFieldAttributeName)) return;
        ...
        private bool IsAutoParameter(SyntaxNodeAnalysisContext context, MemberDeclarationSyntax syntax)
        {
            ...
            bool hasDerivedAttribute = syntax.AttributeLists
                ...
                .Any(attribute =>
                {
                    ...
                    while (current != null)
                    {
                        if (current.ToDisplayString() == Strings.AutoParameterAttributeName)
                        {
                            return true;
                        }
        ...
        private bool IsAutoBundleParameter(SyntaxNodeAnalysisContext context, MemberDeclarationSyntax syntax)
        {
            ...
            bool hasDerivedAttribute = syntax.AttributeLists
                ...
                .Any(attribute =>
                {
                    ...
                    while (current != null)
                    {
                        if (current.Name == Strings.AutoParameterBundleAttributeName)
                        {
                            return true;
                        }
        ...
        private bool IsAutoParameterBundle(ISymbol type)
        {
            ...
            foreach (var att in attributes)
            {
                ...
                while (current != null)
                {
                    if (current.ToDisplayString() == Strings.AutoParameterBundleAttributeName)
                    {
                        return true;
                    }
