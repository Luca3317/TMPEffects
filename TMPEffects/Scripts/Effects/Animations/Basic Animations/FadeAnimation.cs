...
public class FadeAnimation : TMPAnimation
{
    ...
    public override void Animate(ref CharData cData, ref IAnimationContext context)
    {
        ...
        if (ctx.waitingSince == -1 && Mathf.Sign(repeat) != Mathf.Sign(ctx.lastRoc))
        {
            ...
            EffectUtility.BeginWaiting(ref context, out ctx.waitingSince);
        }

        if (EffectUtility.TryFinishWaiting(ctx.isMax ? currentAfterMaxWaitDuration : currentAfterMinWaitDuration, ref context, ref ctx.waitingSince))
        {
